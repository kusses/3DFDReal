...
from pointcept.utils.misc import (
    ...
    #calculate_soft_ap_per_class,
    calculate_true_ap_per_class_with_threshold,
    #calculate_true_ap_per_class,
    make_dirs,
)
...
class TesterBase:
    def __init__(self, cfg, model=None, test_loader=None, verbose=False) -> None:
        ...
        self.writer = SummaryWriter(log_dir=os.path.join(cfg.save_path, "tensorboard_test"))
...
class SemSegTester(TesterBase):
    def test(self):
        #(JY) loss monitoring
        self.ce_loss = torch.nn.CrossEntropyLoss(ignore_index=self.cfg.data.ignore_index)
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # Ensure all_probs is initialized regardless of pred cache
            if self.cfg.data.test.type == "volmeda":
                all_probs = torch.zeros((segment.size, self.cfg.data.num_classes)).cpu()
            ...
            if os.path.isfile(pred_save_path):
                ...
                if self.cfg.data.test.type == "volmeda":
                    probs_save_path = os.path.join(save_path, f"{data_name}_probs.npy")
                    if os.path.exists(probs_save_path):
                        all_probs = torch.from_numpy(np.load(probs_save_path))
                    else:
                        logger.warning(
                            f"[WARN] Cached pred exists, but no cached all_probs found for {data_name}. AP may be invalid.")
                        all_probs = torch.zeros((segment.size, self.cfg.data.num_classes)).cpu()
                ...
                for i in range(len(fragment_list)):
                    ...
                    with torch.no_grad():
                        output_dict = self.model(input_dict)["seg_logits"]  # (n, k)
                        logits = output_dict # raw lostis
                        temperature = 1 # (JY) softmax temperature scaling
                        pred_part = F.softmax(logits / temperature, dim=1)

                        # for val_loss calculation, keep raw logits
                        if i == 0:
                            pred_for_loss = torch.zeros_like(pred)  # shape: [total_points, num_classes]

                        bs = 0
                        for be in input_dict["offset"]:
                            indices = idx_part[bs:be]
                            pred[indices, :] += pred_part[bs:be]
                            if self.cfg.data.test.type == "volmeda":
                                all_probs[indices.cpu(), :] += pred_part[bs:be].cpu()
                            # raw logits save
                            pred_for_loss[indices, :] = logits[bs:be].detach()
                            bs = be
                ...
                # Save prediction results (JY, both .npy files)
                if self.cfg.data.test.type == "volmeda":
                    base_name = data_name.replace(".ply", "")
                    probs_save_path = os.path.join(save_path, f"{base_name}_probs.npy")
                    segment_save_path = os.path.join(save_path, f"{base_name}_segment.npy")
                    np.save(probs_save_path, all_probs.numpy())

                    segment_np = segment.cpu().numpy() if isinstance(segment, torch.Tensor) else segment
                    np.save(segment_save_path, segment_np.astype(np.int32))
                else:
                    np.save(pred_save_path, pred)

                # tensorboard validation loss monitoring
                # pred: shape [N, num_classes]
                # segment: shape [N]
                if isinstance(pred, np.ndarray):
                    pred = torch.from_numpy(pred).cuda()

                segment_tensor = segment if isinstance(segment, torch.Tensor) else torch.from_numpy(segment)
                segment_tensor = segment_tensor.long().to(pred_for_loss.device)

                val_loss = self.ce_loss(pred_for_loss, segment_tensor)
                if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
        ...
        if comm.is_main_process():
            ...
            if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
                ...
                self.writer.add_scalar("Val/mAcc", mAcc, self.epoch)
                self.writer.add_scalar("Val/allAcc", allAcc, self.epoch)
            ...
            if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
                for i in range(self.cfg.data.num_classes):
                    ...
                    self.writer.add_scalar(f"Val/IoU/{class_name}", iou_class[i], self.epoch)

            # (JY) AP calculation only for the volmeda
            # ... inside if comm.is_main_process(): after mIoU logging

            if self.cfg.data.test.type == "volmeda":
                logger.info("=> Calculating per-class AP over all test samples (aggregated) ...")
                ...
                for name, r in record.items():
                    seg_path = os.path.join(save_path, name.replace('.ply', '') + "_segment.npy")
                    prob_path = os.path.join(save_path, name.replace('.ply', '') + "_probs.npy")

                    if os.path.exists(prob_path) and os.path.exists(seg_path):
                ...
                #ap_per_class = calculate_true_ap_per_class(
                #    probabilities=all_probs,
                #    targets=all_segments,
                #    num_classes=self.cfg.data.num_classes,
                #    ignore_index=self.cfg.data.ignore_index
                #)

                ap_per_class = calculate_true_ap_per_class_with_threshold(
                    probabilities=all_probs,
                    targets=all_segments,
                    num_classes=self.cfg.data.num_classes,
                    ignore_index=self.cfg.data.ignore_index
                ...
                logger.info("Val result: mAP {:.4f}".format(mAP))

                # TensorBoard logging
                if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
                    self.writer.add_scalar("Val/mAP", mAP, self.epoch)

                for i in range(self.cfg.data.num_classes):
                    logger.info(
                        "Class_{idx} - {name} AP: {ap:.4f}".format(
                            idx=i,
                            name=self.cfg.data.names[i],
                            ap=ap_per_class[i],
                        )
                    ...
                    if hasattr(self, "writer") and self.writer is not None and hasattr(self, "epoch"):
                        class_name = self.cfg.data.names[i]
                        self.writer.add_scalar(f"Val/AP/{class_name}", ap_per_class[i], self.epoch)
